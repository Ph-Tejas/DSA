...
public:
    int maximumAmount(vector<vector<int>>& coins) {
        ...
        for(int i=1;i<n;i++){
            for(int j=1;j<m;j++){
                if(coins[i][j]<0){
                    dp[i][j][2]=max(dp[i-1][j][2]+coins[i][j],dp[i][j-1][2]+coins[i][j]);
                    dp[i][j][1]=max({dp[i-1][j][1]+coins[i][j],dp[i][j-1][1]+coins[i][j],dp[i-1][j][2],dp[i][j-1][2]});
                    dp[i][j][0]=max({dp[i-1][j][0]+coins[i][j],dp[i][j-1][0]+coins[i][j],dp[i-1][j][1],dp[i][j-1][1]});
                }
                else{
                    dp[i][j][2]=max(dp[i-1][j][2]+coins[i][j],dp[i][j-1][2]+coins[i][j]);
                    dp[i][j][1]=max(dp[i-1][j][1]+coins[i][j],dp[i][j-1][1]+coins[i][j]);
